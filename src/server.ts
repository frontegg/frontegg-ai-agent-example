...
app.post('/api/agent', async (req: Request, res: Response) => {
  try {
    ...
    const agentInstance = await getAgentInstance(); // Get or initialize the agent
    ...
    const statusCode = (error instanceof Error && (error as any).status) ? (error as any).status : 500;
     ...
     if (errorMessage.includes('Agent not initialized')) {
        res.status(503).json({ error: 'Agent initialization failed or is in progress. Please try again later.', details: errorMessage });
     } else {
